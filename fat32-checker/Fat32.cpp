	...
	: filename_(filename)
{
 ...
 void Fat32::readDirectory(const BootSector& bs, const std::vector<uint32_t>& fatTable, uint32_t cluster, const std::string& path) { //кластер, с которого начинается чтение (по умолчанию 0)
 ...
 void Fat32::FixCyclicFile(const File& file, const std::vector<uint32_t>& fatTable, std::vector<uint32_t>& curedFatTable, uint32_t bytesPerSector, uint32_t sectorsPerCluster) {
	 ...
	 // Собираем цепочку, пока не наткнёмся на цикл или конец FAT
	 while (current < fatTable.size() && !visited.count(current) && fatTable[current] != 0x0FFFFFF7 && fatTable[current] != 0x00000000) {
		 ...
		 if (fatTable[current] >= fatTable.size()) {
			 break;  // Выход за пределы таблицы — прекращаем цикл
		 }

		 // Получаем следующий кластер
		 current = fatTable[current];

		 // Проверяем, что текущий кластер не указывает на EOF (если метка EOF уже установлена)
		 if (current == 0x0FFFFFFF) {
			 break;
		 }
	 }

	 // Если мы зациклились, обработаем только нужное количество кластеров
	 for (size_t i = 0; i < chain.size(); ++i) {
		 if (i + 1 < requiredClusters && i + 1 < chain.size()) {
			 ...
			 curedFatTable[chain[i]] = 0x00000000; // Обрезаем лишнее
		 }
	 }
